...
function getEffectivePlayerPool(sport) {
    ...
    // The BOUNDS belong in the key rather than the timeframe string that produced them. The same string resolves to different matchups as a season advances, so keying on the string alone served a stale pool after the week rolled over.
...
// The pool retry after a login lands mid-session. The user does not have to be on the Player tab for it to matter, since My Team needs the same pool for names, ranks and stats.
...
// My Team needs the same weekly data a windowed timeframe asks for, and nothing on that tab used to request it, so a league opened straight into My Team showed every windowed line as a dash until the user visited Player Metrics.
...
export function renderPlayerLeaderboard() {
    ...
    // The LOADED league's sport, never the dropdown's. Every stat map, position map and role group keys off it, so reading the dropdown mid-switch would describe a league that is not on screen yet.
    const sport = AppState.loadedSport;
