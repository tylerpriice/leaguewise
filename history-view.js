...
import {
    ...
    recordText, defaultFranchiseIndex, coverageSentence, runSpanText
} from './history.js';
...
// One per role group, since the two groups do not share columns ( item 1).
const defaultSorts = () => ({ primary: defaultSort(), secondary: defaultSort() });
const emptyCareers = () => ({ status: 'idle', rows: [], loaded: 0, total: 0, years: [], phase: 'pools', refining: 0 });
...
// Registered here rather than called from the fetch path, so this tab cannot be the one somebody forgets on the next league switch ( item 6). History belongs to the league that was on screen, so a new league starts from nothing rather than showing the previous league's franchises while its own seasons load.
registerLeagueView('history', { reset: resetHistoryView });

const yearList = (years) => years.join(', ');

// One line, stated once, wherever a number covers fewer seasons than the league has lived. The alternative is a footnote nobody reads under a total that quietly means something else. The sentence is the owner's own ( item 3) and ships as written; only the year list is computed.
...
// The pennant rail (, owner ruled L2). The champions read vertically down the left, one shape per title, which is the pennant wall stood on its end. The shape follows the sport: baseball hangs felt pennants and hockey raises rafter banners, both the same clip-path construction, per the ruling. Until the Classic theme lands the rail wears the current neutral chrome - this entry builds the structure, P2 brings the dressing. A season with no champion is the season being played, and it hangs greyed rather than being left out. A wall with a gap where this year should be reads as a bug; a grey shape reads as a title nobody has won yet.
function railHtml(seasons, nameOf) {
    ...
    const shapes = [...seasons].sort((a, b) => b.year - a.year).map(season => {
        ...
        // The pennant IS the season drill-down. L2's three zones have no seasons list, and clicking a season to load it in the dashboard is not chrome worth losing - the flag already names the year, so it carries the click the old table row used to.
        return `<div class="lh-flag${tone} lh-season-row" data-year="${season.year}" tabindex="0" role="button"
                     title="${escapeHtml(decided ? how : 'Not decided')} - click to load ${season.year} in the dashboard">
                    <div class="lh-flag-shape">${champ ? buildTeamLogoHtml(champ.name, champ.logo) : ''}</div>
                    <div class="lh-flag-year">${season.year}</div>
                    <div class="lh-flag-name">${escapeHtml(decided ? nameOf(season.championKey) : 'Not decided')}</div>
                </div>`;
    }).join('');
...
function rivalryFactsHtml(detail, meShort, oppShort, titles) {
    ...
    // The coordinate first, then the marker, then who won - the owner's ruled order ( item 1). Matchup is capitalised to match the run span beside it, which has always written it that way.
    ...
    // Fixed order, and the depth count says how many of them the height affords ( item 4).
    const extras = {
        longest: () => {
...
function rivalryCardHtml(detail, meName, oppName, titles, shorts) {
    ...
    if (!played) {
        // Two states, not one. A pair with postseason meetings and no regular-season ones has met - the record just does not count those games ( item 1), and saying they never played would be false.
...
function gridHtml(pairs, franchises, countedYears, records) {
    ...
    // NEVER-MET ROWS LAST ( item 2). They carry no bar and no record, so a muted row sitting between two real rivalries breaks the column the eye is reading down. Met opponents keep the order they already had among themselves - a stable partition, not a re-sort.
    ...
    // The owner's own sentence, with both year lists computed ( item 6).
...
// Tier 2 (M3). The union of every category ever scored, each column saying how much of the league's life it covers. Marks rather than words ( item 2): the cell answers one yes-or-no question, and 16 columns of the word "Scored" reads as a wall of text where a column of ticks reads at a glance. The cross is the quiet colour, not the danger one - a category a season did not score is a fact about the league's history, not a fault.
function categoryTableHtml(union, seasons) {
    ...
    // No coverage count under the name and no caption over the table ( items 1 and 3). Both said what the column of marks already says: the reader can see which seasons scored it by looking down the column, and "2 of 3" was the same fact counted for them. The career table keeps its coverage notes, because that one has no marks to read them off.
    const head = union.stats.map(s => `<th>${escapeHtml(map[s.statId] || s.statId)}</th>`).join('');
    // The season's FORMAT beside its year ( item 7). A row of ticks means something different in a roto season than in a head-to-head one, and the reader had no way to tell which they were looking at without going back to the rail.
    ...
    // The row pitch comes from the season count, not from the table being told to be tall ( item 3). categoryRowHeight is pure and tested; all this does is hand the answer to CSS.
...
// The season still being PLAYED, which is not the same as the newest one ( item 5). A finished league's newest season is finished too, and saying its careers keep moving was wrong on every league the owner opened. Newest-and-unfinished, or nothing.
function liveYear() {
...
function careerBlockHtml() {
    ...
    const live = liveYear();
    const note = `Totals over ${years.length} ${years.length === 1 ? 'season' : 'seasons'}: ${yearList(years)}.`
        + (live ? ` ${live} is still being played, so these totals keep moving.` : '');
    let inner = '';
    if (c.status === 'loading') {
        inner = `<div class="player-loading">Loading players ${c.loaded} of ${c.total}</div>`;
    } else if (c.status === 'error') {
        inner = `<div class="player-loading">No season's players could be read.</div>`;
    } else if (c.status === 'done') {
        inner = careerTableHtml(c.rows, years);
    } else {
        inner = '<div class="player-loading">Loading</div>';
    }
    // THE TABLE STAYS ON SCREEN WHILE THE LOG LANDS ( item 2). Attribution starts from the draft, which is one request a season, and the transaction stints patch in per season behind it. The line says which part is still moving rather than hiding the table behind a counter - and it says it about the FRANCHISES column only, because nothing else on the row changes.
    const refining = c.status === 'done' && c.refining
        ? `<div class="lh-note lh-refining">Franchises are from the draft so far. ${c.refining} ${c.refining === 1 ? 'season is' : 'seasons are'} still reading the transaction log.</div>`
        : '';
    return `<div class="lh-block lh-block-fill">
                <div class="lh-note">${escapeHtml(note)}</div>
                ${refining}
                ${inner}
            </div>`;
}
...
function careerGroupHtml(label, rows, statIds, groupKey, union, years) {
    ...
    const body = sortCareers(rows, { key: sort.key, dir: sort.dir, rateSpecs: specs }).map(row => {
        // A COUNT with the names on hover ( item 2). The names themselves were the widest column in the table for a fact most rows answer with "one", and a player who has been round the league four times is the interesting case - which is what a number says at a glance.
...
function careerTableHtml(rows, years) {
    ...
    // ONE GROUP AT A TIME behind pills (, mock C1), which is My Team's Categories/Schedule face pattern reused rather than re-invented - same interaction, same chrome, same classes. Stacked, the two tables split a band that was already the shortest thing on the tab and neither got enough rows to be worth scrolling. One of them at full height is a table you can actually read, and the pill carries the count so the other group is never hidden, only closed.
...
async function ensureCareers(container, key) {
    ...
    // Rebuilt from the pools already in hand every time attribution improves. buildCareers is pure and the pools are cached, so this is arithmetic over data in memory - no refetch, and the sort, the pills and the scroll position all come back the same way they would on any render.
    const attribute = (owners, refining) => {
        ...
        state.careers = { status, rows, loaded: years.length, total: years.length, years, phase: 'done', refining };
        render(container);
    };

    // WHO HELD EACH PLAYER ( item 5), now draft-first ( item 2). The draft is one request a season and lands almost immediately; the per-period log is ~190 a season and patches in behind it. The live season is free when the Roto Race has already harvested it.
    ...
    const owners = await loadHistoryOwnership(AppState.loadedSport, AppState.apiData.id, years, state.payloads, {
        liveSeason: live,
        // The loader says which seasons still owe a transaction log, so the note counts the real remainder rather than assuming every season needs one - a cached season, or the live one the Roto Race already harvested, is finished before this fires.
        onDrafts: (draftOwners, pendingYears) => {
            outstanding = pendingYears.length;
...
function render(container) {
    ...
    // Every season, always. The one-season view the pills used to offer is gone with them ( item 4) - the rivalry card carries a bar per season, so a year is something the reader reads rather than something they have to ask for first.
...
export async function renderHistoryTab() {
    ...
    // NEVER anchored to the loaded year ( item 3). What the dashboard is showing has nothing to do with how long the league has existed.
