...
// EVERY franchise that ever held a player, for one season. The careers table asks a different question from the roto race above it: not "who owned the player on day 87" but "who ever owned them at all", because the ruling is that any stint counts however short - a player drafted, held a week and dropped played for that franchise. Reads the same change points buildRosterTimeline produces, so a drop (teamId 0) contributes nothing and a re-add to the same team cannot double it. Order is first-held first, which is the order a career row reads best in: the franchise that drafted the player leads.
