...
test('bug, reproduced: the OLD season-average basis saturates: a real bad week still scores ~89 against smoothed part-timer averages', () => {
...
test('buildStartedTimeline: benched, IR, unrostered, and never-seen all fall through to nobody', () => {
    // The per-day fallback: any day a player is not in a starting slot on some team credits nobody, so the race simply skips it (whichever fallback tier is active never invents a crediting team).
    const tl = buildStartedTimeline({
